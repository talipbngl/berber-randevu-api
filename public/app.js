// app.js - Frontend Logiği (Tek DOMContentLoaded + sağlamlaştırıldı)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Eğer sayfada beklenen elemanlar yoksa (yanlış sayfada bu js yüklenmiş olabilir)
  if (!form || !dateInput || !slotListDiv || !selectedTimeInput || !nextStep1Button || !nextStep2Button) {
    console.warn('app.js: Gerekli DOM elemanları bulunamadı. Bu sayfa app.js için uygun olmayabilir.');
    return;
  }
  ...
  // ===============================
  // Cancellation (İptal) — senin kodunu aynı bıraktım
  // ===============================
  function renderCancellableAppointments(appointments) {
    if (!cancellationResults) return;
  ...
  if (prevStep2Button) prevStep2Button.addEventListener('click', () => updateStep(1));
  if (prevStep3Button) prevStep3Button.addEventListener('click', () => updateStep(2));

  if (openCancellationBtn && cancellationModal && cancellationForm && cancellationResults) {
  ...
  if (closeCancellationBtn && cancellationModal && cancellationForm) {
  ...
  if (cancellationModal && cancellationForm) {
  ...
  if (cancellationForm && cancellationResults && listAppointmentsBtn && cancelPhoneInput) {
